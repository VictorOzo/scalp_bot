...
def _query_trades(
    ...
    *,
    ...
    if mode:
        where.append("json_extract(meta_json, '$.mode') = ?")
        params.append(mode.upper())
    if command_id is not None:
        where.append("json_extract(meta_json, '$.command_id') = ?")
    ...
    rows = conn.execute(
        f"""
        SELECT id,time_open_utc,time_close_utc,pair,direction,units,entry_price,exit_price,result,meta_json
        FROM trades
        {where_clause}
        ORDER BY id DESC
        LIMIT ?
